...
## The Adaptive Governance Specification Principle

AAO treats governing documents not as static artifacts but as **adaptive governance specifications** — machine-readable sources of truth that enforce rules at runtime and evolve from telemetry generated by real sessions.

A static governance document describes rules once and hopes they stay current. An adaptive governance specification enforces rules at each action, collects failure signals, and refines itself based on what those signals show.

> *This principle extends the "living specification" concept from software architecture — the idea that specifications should be active enforcement mechanisms rather than passive documentation — and applies it to the governance of autonomous AI systems. For the original architectural framing, see Enrico Piovesan, "Living Specifications: The Future of Software Architecture," Mastering Software Architecture for the AI Era, Part 18 (2025).*

### Telemetry-Driven Evolution

Every AAO session generates signals that feed back into the governing document:

| Signal | Source | What It Informs |
|--------|--------|-----------------|
| SQS — Session Quality Score | Session close | Whether governance rules are producing quality outcomes |
| REC — Rule Enforcement Count | Per-session violations | Which rules trigger most — candidates for clarification or removal |
| UAC — Unauthorized Action Count | Scope violations | Whether scope discipline is holding |
| task_classification | Extended data field | Whether tier routing is accurate and cost-efficient |

A rule that triggers repeated violations is a rule that needs sharpening. A governance spec that produces consistent GREEN confidence scores has reached maturity. The loop is: **specification → enforcement → telemetry → refinement → better specification**.

### The Three-Tier Agentic Model

AAO formalizes three tiers of AI participation. Each tier has defined authority, minimum capability requirements, and a specific role in the governance loop. No model names are specified — any model meeting the minimum capability threshold for its tier is valid. The architecture is provider-agnostic.
...
### Atomic Spec — The Tier 2 Handover Contract

Before any Tier 3 task begins, Tier 2 must produce an Atomic Spec. This is the handover contract that eliminates ambiguity for smaller models:

| Element | Content | Why It Matters |
|---------|---------|----------------|
| Constraint boundaries | What the implementer cannot do | Prevents over-reach, library sprawl, memory violations |
| Interface contract | Exact input/output types, JSON schema if applicable | Small models excel at filling pre-defined structures |
| Context injection | One file or module only — not the full codebase | Long-range dependencies confuse smaller models |
| Unit test as requirement | Test suite must exist before implementation begins | Enforces TDD structurally — not as a suggestion |

### Watchdog Protocol

After every Tier 3 module completes, the output is submitted to Tier 1 or Tier 2 with a **Decision Log** — a brief explanation of choices made during implementation. The Watchdog checks:

1. Does the output conform to the Atomic Spec contract?
2. Were any constraint boundaries violated?
3. Do any decisions in the log contradict the ARCHITECTURE.md?

The next module does not begin until Watchdog approves. The governance spec is not a starting document — it is an active constraint checked at every module boundary.
...
│
├── audit/
│   ├── CLAUDE_CODE_AUDIT.md           ← 180-point Claude Code compliance scoring framework
│   └── CLAUDE_CODE_STATE_ASSESSMENT.md ← Dated compliance snapshots
│
├── remediation/
│   └── CLAUDE_CODE_GAP_REMEDIATION.md ← Task list to hand Claude Code for gap closure
│
├── commands/
